...
void processor::triggerer_thread(std::atomic<bool> & running, std::atomic<bool> & stopped)
{
    ...
    debug::log("[CPU] Starting execution of ", ops_num, " operations per second.\n");
    debug::log("[CPU] Frequency set to: ", frequencyHz, " Hz.\n");
    debug::log("[CPU] Period per operation: ", period_ns.count(), " nanoseconds.\n");
...
void processor::collaboration()
{
    ...
    debug::log("[CPU] CPU time frame precision collaboration done.\n");
}
